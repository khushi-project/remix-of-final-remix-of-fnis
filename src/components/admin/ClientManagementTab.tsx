...
import { Trash2, Plus, UserPlus, X, Link, Loader2, Users } from 'lucide-react';
import { motion } from 'framer-motion';
import { useAdminData } from '@/context/AdminContext';
import type { AdminClient } from '@/hooks/useAdminDashboard';
import {
...
const ClientManagementTab = () => {
  ...
  const { clients, trainers, trainerClientMappings, dietPlans, addClient, removeClient, assignClientToTrainer, unassignClient } = useAdminData();
  ...
  const getClientTrainer = (clientId: string) => {
    const mapping = trainerClientMappings.find(m => m.clientId === clientId);
    if (!mapping) return undefined;
    return trainers.find(t => t.id === mapping.trainerId);
  };

  const getClientDietPlans = (clientId: string) => dietPlans.filter(p => p.clientId === clientId);

  const handleAssignTrainer = (clientId: string, trainerId: string) => {
    const client = clients.find(c => c.id === clientId);
    if (!client) return;
    // Remove old assignment first
    const oldMapping = trainerClientMappings.find(m => m.clientId === clientId);
    if (oldMapping) unassignClient(oldMapping.trainerId, clientId);
    assignClientToTrainer(clientId, trainerId, client.name, client.email);
    setAssigningClient(null);
    toast({ title: 'Trainer assigned', description: 'Client assigned to trainer.' });
  };

  const handleUnassignTrainer = (clientId: string) => {
    const mapping = trainerClientMappings.find(m => m.clientId === clientId);
    if (mapping) unassignClient(mapping.trainerId, clientId);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        <div className="divide-y divide-border">
          {clients.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Add a client using the form above.</p>
            </div>
          ) : (
            clients.map(client => {
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {trainer && (
                        <button
                          ...
                          className="rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-destructive transition-colors hover:border-destructive hover:bg-destructive/10"
                        >
                          Unassign
                        </button>
                      )}
      ...
      {/* Client Detail Modal */}
      {selectedClient && (() => {
        const trainer = getClientTrainer(selectedClient.id);
        const clientDietPlans = getClientDietPlans(selectedClient.id);
        return (
